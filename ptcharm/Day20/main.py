...
# 매개변수 1 이면 모든 움직임이 즉시 화면에 반영(한번에 출력이 아닌 하나씩 애니메이션 효과처럼 출력) / 0 이면 애니메이션 효과 없이 한번에 화면에 출력
screen.tracer(0)
...
while game_is_on:
    # update - 네모가 생성도는 과정은 보여주지 않으면서 네모가 생성된 결과만 보여준다
    screen.update() # tracer 메소드와 짝 / 최종 그림을 화면에 한 번에 업데이트
    time.sleep(0.1) # 움직임을 0.1 초의 텀으로 출력
